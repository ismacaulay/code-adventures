<script lang="ts">
  ...
  import { VoxelContainer, VoxelChunk } from 'toolkit/geometry/voxel';
  ...
  function voxelizeMesh(
    ...
    // const vertices = new Float64Array(mesh.triangles.length * 3);
    // let tri: number;
    // for (let i = 0; i < mesh.triangles.length; ++i) {
    //   tri = mesh.triangles[i];
    //
    //   vertices[i * 3] = mesh.vertices[tri * 3];
    //   vertices[i * 3 + 1] = mesh.vertices[tri * 3 + 1];
    //   vertices[i * 3 + 2] = mesh.vertices[tri * 3 + 2];
    // }
  ...
  function generateVoxelMesh(
    mesh: { vertices: number[] | Float32Array | Float64Array; triangles: Uint32Array },
    voxelSize: vec3,
  ) {
    ...
    console.log(octree);
    return octree;
    // const voxels = VoxelContainer.create();

    // const chunkBlockCount = 16;
    // const chunkSize = vec3.fromValues(
    //   chunkBlockCount * voxelSize[0],
    //   chunkBlockCount * voxelSize[1],
    //   chunkBlockCount * voxelSize[2],
    // );
    //
    // const blockCount = vec3.create();
    // vec3.sub(blockCount, aabb.max, aabb.min);
    // vec3.set(
    //   blockCount,
    ...
    //   Math.ceil(blockCount[0] / chunkBlockCount),
    //   Math.ceil(blockCount[1] / chunkBlockCount),
    //   Math.ceil(blockCount[2] / chunkBlockCount),
    // );
    //
    ...
    //         for (let k = 0; k < chunkBlockCount; ++k) {
    //           for (let j = 0; j < chunkBlockCount; ++j) {
    //             for (let i = 0; i < chunkBlockCount; ++i) {
    //               vec3.set(
    //                 blockAABB.min,
