...
class Framebuffers : public Tutorial
{
public:
    ...
    std::shared_ptr<tk::engine::Framebuffer> framebuffer_;

    std::shared_ptr<tk::engine::Shader> quad_shader_;
    std::shared_ptr<tk::engine::VertexArray> quad_va_;

    Framebuffers()
        : Tutorial("Framebuffers")
    {}
    ...
    void attach(tk::engine::Engine& engine) override
    {
        ...
        auto perspective_camera =
            std::make_shared<tk::engine::PerspectiveCamera>(
                45.0f, window.aspect(), 0.1f, 100.0f);
        ...
        shader_ = tk::engine::Shader::from_file(
            "assets/shaders/framebuffer_scene.glsl");
        ...
        shader_->bind();
        cube_texture_ = tk::engine::Texture2D::create("assets/container.jpg");
        floor_texture_ =
        ...
        framebuffer_ =
            tk::engine::Framebuffer::create(window.width(), window.height());

        quad_shader_ =
            tk::engine::Shader::from_file("assets/shaders/quad.glsl");
        quad_va_ = tk::engine::VertexArray::create();
        vb = tk::engine::VertexBuffer::create(
            QUAD_VERTICES.data(), sizeof(float) * QUAD_VERTICES.size());
        vb->set_layout({ { tk::engine::ShaderDataType::Float2, "a_position" },
                         { tk::engine::ShaderDataType::Float2, "a_uv" } });
        quad_va_->add_vertex_buffer(vb);
    }
    ...
    void update(float delta) override
    {
        ...
        tk::engine::RenderCommand::set_clear_color({ 1.0f, 1.0f, 1.0f, 1.0f });
        tk::engine::RenderCommand::clear();

        tk::engine::RenderCommand::enable_depth_test(false);
        framebuffer_->bind_texture();
        tk::engine::Renderer::submit(quad_shader_, quad_va_);
