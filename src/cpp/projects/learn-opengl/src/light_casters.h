...
class LightCasters : public Tutorial
{
public:
    ...
    LightCasters()
        : Tutorial("LightCasters")
    {}
    ~LightCasters() = default;
    ...
    void attach(tk::engine::Engine& engine) override
    {
        ...
        cube_shader_ =
            tk::engine::Shader::from_file("assets/shaders/light_casters.glsl");
        cube_shader_->bind();
        // cube_shader_->set_uniform_vec3("u_light.position", light_pos);
        cube_shader_->set_uniform_float("u_light.constant", 1.0f);
        cube_shader_->set_uniform_float("u_light.linear", 0.09f);
        cube_shader_->set_uniform_float("u_light.quadratic", 0.032f);

        cube_shader_->set_uniform_vec3("u_light.ambient", { 0.1f, 0.1f, 0.1f });
    ...
    void update(float delta) override
    {
        ...
        cube_shader_->set_uniform_vec3("u_light.position",
                                       camera_controller_->position());
        cube_shader_->set_uniform_vec3("u_light.direction",
                                       camera_controller_->front());
        cube_shader_->set_uniform_float("u_light.cutoff",
                                        glm::cos(glm::radians(12.5f)));
        cube_shader_->set_uniform_float("u_light.outer_cutoff",
